...
Deno.test('Valid identifier names', async () => {
    ...
    assert(result1.reason === 'finish', `Expected finish, but got ${result1.reason}`)
    const result2 = await yaksok(`야용이 = 20`)
    assert(result2.reason === 'finish', `Expected finish, but got ${result2.reason}`)
    const result3 = await yaksok(`ㄱ자_전선 = 20`)
    assert(result3.reason === 'finish', `Expected finish, but got ${result3.reason}`)
    const result4 = await yaksok(`내이름은ㄴ이야 = 20`)
    assert(result4.reason === 'finish', `Expected finish, but got ${result4.reason}`)
    const result5 = await yaksok(`_사용하지않음 = 20`)
    assert(result5.reason === 'finish', `Expected finish, but got ${result5.reason}`)
})
