...
import { CodeFile, YaksokSession } from '../../core/mod.ts'
...
function getMessages(codeFile: CodeFile) {
    return codeFile.prepareErrors.map((e) => e.message)
}

Deno.test('이고 붙여쓰기 - 일반 표현식에서 hint 포함', async () => {
    // 반복문/조건문 바깥에서 이고가 붙어있을 때 NotDefinedIdentifierError에 hint가 붙어야 함
    const codeFile = await runCode(`결과이고 보여주기`)
    const messages = getMessages(codeFile)
    assert(
