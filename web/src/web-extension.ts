...
class HTMLInputValue extends InstanceValue {
    ...
    updateValue() {
        // 토큰은 임시로 빈 배열
        this.scope.setLocalVariable('값', new StringValue(this.element.value || ""), []);
    }
...
`
    ...
    async init() {
        ...
        this.session.eventCreation.sub('click', (args, callback, terminate) => {
            ...
            if (target instanceof HTMLButtonValue || target instanceof HTMLTextValue || target instanceof HTMLBoxValue) {
    ...
    executeFFI(code: string, args: FunctionInvokingParams): ValueType {
        ...
        if (payload.type === 'append-child') {
            ...
            if (parent instanceof HTMLBoxValue) {
                ...
                if (child instanceof HTMLButtonValue) childElement = child.element;
                else if (child instanceof HTMLTextValue) childElement = child.element;
        ...
        if (payload.type === 'remove-element') {
            const target = params['개체'];
            if (target instanceof HTMLBoxValue || target instanceof HTMLTextValue || target instanceof HTMLButtonValue) {
