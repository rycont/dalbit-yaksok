...
- 멤버 조회/설정 해석: [`/core/node/class/member-runtime.ts`](/core/node/class/member-runtime.ts)
- 인스턴스/검증용 스코프 생성: [`/core/node/class/validation-instance.ts`](/core/node/class/validation-instance.ts)
- 생성자 arity 해석: [`/core/node/class/constructor.ts`](/core/node/class/constructor.ts)
- 클래스 선언 검증 유틸: [`/core/node/class/declare-validation.ts`](/core/node/class/declare-validation.ts)
- 인스턴스 생성자 선택 유틸: [`/core/node/class/new-instance-constructor.ts`](/core/node/class/new-instance-constructor.ts)
...
이 타입들은 [`/core/node/class/core.ts`](/core/node/class/core.ts)에 정의되어 있습니다.
...
생성자 선택 규칙은 [`/core/node/class/new-instance-constructor.ts`](/core/node/class/new-instance-constructor.ts)에 있습니다.
...
- 멤버 변수 조회는 현재 레이어에서 `memberLookupRootScope`까지 체인을 타고 탐색합니다.
- 멤버 메서드도 같은 체인에서 탐색합니다.
- 전역 변수/전역 함수로 폴백하지 않습니다.

`FetchMember`는 변수 조회 실패 시 **무인자 멤버 메서드 자동 호출**을 시도합니다. 둘 다 없으면 `MemberNotFoundError`를 발생시킵니다.
...
클래스 헤더 파싱은 [`/core/prepare/parse/dynamicRule/functions/declare-rule/class-header.ts`](/core/prepare/parse/dynamicRule/functions/declare-rule/class-header.ts)에서 처리합니다.

- `클래스, A`
- `클래스, B(부모)`
- `클래스, C, 부모`

세 형태를 모두 지원하며, 부모 지정 문법 오류는 `UnexpectedTokenError`/`UnexpectedEndOfCodeError`로 보고됩니다.
